...
CREATE OR REPLACE FUNCTION estimated_extent(text,text,text) RETURNS box2d AS
	'$libdir/postgis-1.4', 'LWGEOM_estimated_extent'
...
CREATE OR REPLACE FUNCTION postgis_scripts_build_date() RETURNS text
	AS 'SELECT ''2009-12-11 19:07:53''::text AS version'
	LANGUAGE 'sql' IMMUTABLE;
...
CREATE OR REPLACE FUNCTION CheckAuthTrigger()
	RETURNS trigger AS 
	'$libdir/postgis-1.4', 'check_authorization'
	LANGUAGE C;

CREATE OR REPLACE FUNCTION GetTransactionID()
	RETURNS xid AS 
	'$libdir/postgis-1.4', 'getTransactionID'
	LANGUAGE C;
...
--    RETURNS bytea
--    AS '$libdir/postgis-1.4','LWGEOM_AsShape'
--    LANGUAGE 'C' IMMUTABLE STRICT; 
